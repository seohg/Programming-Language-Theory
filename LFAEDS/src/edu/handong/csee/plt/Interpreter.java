...
public class Interpreter {
	...
	public Value interp(AST ast, DefrdSub ds) {
		...
		if(ast instanceof Dsfun) {
			...
			return new ClosureV(dsfun.getP(),dsfun.getB(),ds);
		}
		...
		if(ast instanceof App) {
			...
			ASub asub = new ASub(fval.getParam(),aval,fval.getDs());
			return interp(fval.getBody(), asub);
		}
		
		if(ast instanceof DsApp) {
			DsApp dsapp = (DsApp)ast;
			Strict strict = new Strict();

			ClosureV fval = (ClosureV)interp(dsapp.getF(),ds);
			Value aval = interp(dsapp.getA(),ds);

			ASub asub = new ASub(fval.getParam(),aval,ds);
			return interp(fval.getBody(), asub);
		}
